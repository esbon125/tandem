...
sd_create_pin_slices -sd_name ${sd_name} -pin_name {MSS_WRAPPER_0:MSS_INT_F2M} -pin_slices {[19:16]}
sd_create_pin_slices -sd_name ${sd_name} -pin_name {MSS_WRAPPER_0:MSS_INT_F2M} -pin_slices {[20:20]}
sd_create_pin_slices -sd_name ${sd_name} -pin_name {MSS_WRAPPER_0:MSS_INT_F2M} -pin_slices {[58:21]}
...
sd_connect_pins_to_constant -sd_name ${sd_name} -pin_names {MSS_WRAPPER_0:MSS_INT_F2M[19:16]} -value {GND}
sd_connect_pins_to_constant -sd_name ${sd_name} -pin_names {MSS_WRAPPER_0:MSS_INT_F2M[58:21]} -value {GND}
...
# FIC_1_AXI4_TARGET (Fase 6b): no longer unused -- connected below to
...
# FIC_1_ACLK (Fase 6b): fed by mpeg2video's own mem_clk (via
# FIC_3_PERIPHERALS_0:MEM_CLK_MPEG2FPGA, see mpeg2fpga_apb_peripheral.v),
# NOT by CLOCKS_AND_RESETS_0:FIC_1_CLK -- mem_req_rd_*/mem_res_wr_* are
# already in mpeg2video's own mem_clk domain (framestore.v's dual-clock
# FIFOs only cross clk<->mem_clk, nothing further), so the AXI4 transactions
# mem2axi_bridge issues on this same net must run on that exact clock, not
...
# Fase 5b: mpeg2fpga replaces the 7-segment display SPI at this FIC_3 slot
# (see docs/bringup/05_mss_apb_bridge_tdd.md). Reuses the same 50 MHz
# reference clock as CLOCKS_AND_RESETS (PF_CCC_C0 inside mpeg2video was
# generated for that frequency) and a free MSS_INT_F2M bit (0-1, 3, 5,
# 16-58 are unused by this design; picked 20, clear of both the low
# peripheral-IRQ cluster and the high MIV_IHC cluster).
sd_connect_pins -sd_name ${sd_name} -pin_names {"REF_CLK_50MHz" "FIC_3_PERIPHERALS_0:REF_CLK_MPEG2FPGA" }
sd_connect_pins -sd_name ${sd_name} -pin_names {"FIC_3_PERIPHERALS_0:MPEG2FPGA_INTERRUPT" "MSS_WRAPPER_0:MSS_INT_F2M[20:20]" }
